		...
		match action:
			Task.TASK_LIST:
				if bod and bod.keys().size() > 0:
					error = bod[0].error
					task_list_error.emit(error.code, error.status, error.message)
				else:
					task_list_error.emit(1, 0, "Unknown error when returning from list")
			Task.TASK_QUERY:
				if bod and bod.keys().size() > 0:
					error = bod[0].error
					task_query_error.emit(error.code, error.status, error.message)
				else:
					task_query_error.emit(1, 0, "Unknown error when returning from query")
			_:
				if bod:
					error = bod.error
					task_error.emit(error.code, error.status, error.message)
				else:
					task_error.emit(1, 0, "Unknown error when returning from task")
